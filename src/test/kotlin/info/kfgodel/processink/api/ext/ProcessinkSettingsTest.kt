...
class ProcessinkSettingsTest : KotlinSpec() {
  override fun define() {
    describe("a settings api") {
      val applet by let { mockk<ProcessingApplet>(relaxed = true) }
      val settings by let {
        val settings = spyk<ProcessingApplet>()
        every { settings.applet() } returns applet() // Use fake applet to intercept calls to Processing
        settings
      }
